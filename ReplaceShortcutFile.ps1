...
function CleanupShortcut([string] $TargetName) {
    ...
    if ( [System.IO.Path]::GetDirectoryName($TargetName) -eq $srcpath ) {
        $null = Write-Host "削除`t代替元不要(自フォルダへのリダイレクト)`t""$TargetName"""
        Remove-Item $TargetName
        return
    }
    # 不正ショートカットファイル
    if ( ($srcpath -eq "") -and ($srcwork -ne "") ) {
        $null = Write-Host "修正`t代替元不正(ターゲット無/ワーキング有)`t""$TargetName"""
        # 修正後に処理を継続
    ...
    if ( $srcexist -eq $true ) {
        $null = Write-Host "無視`t処理不要`t""$TargetName"""
        return
    }
    # スキップ対象チェック
    if ( (isSkipPath $srcpath) -or (isSkipPath $srcwork) ) {
        $null = Write-Host "無視`t無視リストに該当`t""$TargetName"""
        return
    }
    # 代替候補パス生成
    ...
    if ( ($srcpath -eq $dstpath) -and ($srcwork -eq $dstwork) ) {
        $null = Write-Host "失敗`t代替先不変`t""$TargetName"""
        return 
    }
    if ( ($dstpath -eq "") -and ($dstwork -eq "") ) {
        $null = Write-Host "失敗`t代替先不正(ターゲット無/ワーキング無)`t""$TargetName"""
        return
    }
    if ( ($dstpath -eq "") -and ($dstwork -ne "") ) {
        $null = Write-Host "失敗`t代替先不正(ターゲット無/ワーキング有)`t""$TargetName"""
        return
    }
    # 代替候補パス不在
    $dstexist = $true
    ...
    if ( ($dstexist -eq $false)  ) {
        $null = Write-Host "失敗`t代替先不在`t""$TargetName"""
        return
    }
    # 入替
    ...
    $lnk.Save()
    $null = Write-Host "成功`t""$TargetName"""
}
