    ...
    switch ($Mode) {
        "PSH5 CUI" {
            if ($ename.ToLower() -eq ".ps1") {
                ...
                $text = $text + """$($ENV:SystemRoot)\system32\WindowsPowerShell\v1.0\powershell.exe"" -NoProfile -ExecutionPolicy Bypass -File "".\$tpath"" %*" + "`r`n"
                $text = $text + "popd" + "`r`n"
            }
        }
        "PSH5 GUI" {
            if ($ename.ToLower() -eq ".ps1") {
                ...
                $text = $text + "pushd %~dp0" + "`r`n"
                $text = $text + "chcp 65001" + "`r`n"
                $text = $text + """$($ENV:SystemRoot)\system32\WindowsPowerShell\v1.0\powershell.exe"" -NoProfile -WindowStyle hidden -ExecutionPolicy Bypass -File "".\$tpath"" %*" + "`r`n"
                $text = $text + "popd" + "`r`n"
            }
        }
        "PSH5 ISE" {
